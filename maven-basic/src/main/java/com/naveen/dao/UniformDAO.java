...
public class UniformDAO {
	...
	public List<UniformBean> getAssignedUniform(int schId, String cls,
			String sex) {
		String sql = "select u.unid, u.uname, u.short_code from uniform u, "
				+ "ASSIGNUNIFORM AU where u.unid = au.unid and "
				+ "au.schid = ? and class = ? and sex=? ";
	...
	public void insertUniform(int schId, String cls, String sex,
			String[] uniformIds, int uid) {
	 ...
	 * @param sex
	 * @return
	 * @see  tax added by naveen
		 added ordersOrder by Naveen 19JAN2016
		 changed again on 26 JULY 2017 after GST 
		 taken off tax  in sql query and got cgst, sgst, igst 
	 */
	public ArrayList<ArrayList<Object>> getAssignedPriceUniform(int schId,
			String cls, String sex) {
	...
	public boolean insertPrice(double price, int auId, double cgst, double sgst, double igst, int ordersOrder, double courierCost) {
	 ...
	 */
	public boolean updateUniformHouse(int schId, String cls, String sex,
			int uniformId, int houseId) {
	...
	// this function is used in takemesurement3.jsp, for displaying uniform shortcode
	 ...
	 * @param unId
	 * @return
	 * @see this method is used in TakeMesurement3.jsp for showing house, since
	 *      you can get house number from combination of sex, class, school,
	 *      uniformId
	 */
	...
	/**
	 * this function is used in TakeMesurementController.java for sending unid
	 * take measurement order
	 * 
	 * @return map
	 * @see : the map will contain uniformId as key and list mesurement ids
	 *      which is used to do request.getparmeter of the parmeters passed.
	 */
...
//			gc.ps1.setString(4, color);
	...
	public int insertStudentMesurement(int auid, int stid, String color, int uid) {
		...
		//System.out.println("auid " + auid +", stid " + stid +", color " + color +", smid " + smid);
...
//	Integer.parseInt(req), mob.getmId(), smid 
	 ...
	 * @since Oct 09 2016 
	 * @param auid
	 * @see Called From DeleteAssignedUniformController in turn from DeleteUniformForm.jsp
	 * 
	 */
	public  boolean deleteExistingUniform(int auid) {
		boolean flag = false;
		String sql1="delete from studentmesurementdetails where smid in (select smid from studentmesurement where auid = "+auid+")";
	...
	*//**
	...
	public List<Integer> getAuid(int schoolId, String cls, String sex){
		String sql = "select auid  from assignuniform where SCHID = ? and class=? and sex =?";
