...
import com.naveen.beans.AuditReportBean;
...
public class AuditDAO {
	...
	public ArrayList<AuditReportBean> getAuditReportBetweenDates(
...
/*		select si.invoiceno, si.date, so.total as 'BILL AMOUNT',
		...
		GROUP_CONCAT(if(tax='14.50',taxamount,0)) AS 'Tax@14.50', so.grandtotal
		from orderdetail od, studentinvoice si, studentorder so where 
			od.oid = si.oid and od.oid = so.oid and si.oid = so.oid and 
			od.oid in 
			(select oid from studentinvoice where date between '2014-01-01' and '2016-04-10')
			group by od.oid
			order by si.invoiceno;
*/
		...
		// for CGST 
		List<Double> distinctList = getDistinctNewTaxBetweenRange(startDate,
				endDate, "cgstax");

		System.out.println("CGST : " +distinctList);
		for (Double temp : distinctList) {
			sql += "GROUP_CONCAT(if(cgstax='" + temp + "',cgstaxamount,0)) AS 'TAX@"
					+ temp + "',";
		}

		
		// for SGST 		
		distinctList = getDistinctNewTaxBetweenRange(startDate,
				endDate, "sgstax");
		System.out.println("SGST : " +distinctList);
		
		for (Double temp : distinctList) {
			sql += "GROUP_CONCAT(if(sgstax='" + temp + "',sgstaxamount,0)) AS 'TAX@"
					+ temp + "',";
		}
		...
		for (Double temp : distinctList) {
			sql += "GROUP_CONCAT(if(igstax='" + temp + "',igstaxamount,0)) AS 'TAX@"
					+ temp + "',";
		}
		...
		sql += " from orderdetail od, studentinvoice si, studentorder so where od.oid = "
				...
				+ "from studentinvoice where date between ? and ?) group by "
				+ "od.oid order by si.invoiceno";
		
		// dont use logger here as sql command will contain single quotes ', this will cause an error 
...
//				hashMap.put(gc.rs1.getString(1), perProductTax);
			...
			// 	System.out.println("2015/16/CHIN/10  - " +hashMap.get("2015/16/CHIN/10").get("5.5"));
			/*	try {
					Thread.sleep(1000);
	...
	public List<Double> getDistinctTaxBetweenRange(String startDate,
			String endDate) {
	...
	public List<Double> getDistinctNewTaxBetweenRange(String startDate,
			String endDate, String taxType) {
		String sql = "select distinct "+taxType+" from orderdetail where oid in	(select oid from studentinvoice where date between ? and ?)";
	...
	public static void main(String[] args) {
		...
		System.out.println("Summary : Grand Total "+ map.get("grandTotal") +", Total Bill Amount" + map.get("totalBillAmount") +", Tax @5.5 :" +map.get("5.5") +", Tax@14.5: " + map.get("14.5") );
