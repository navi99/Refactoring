 ...
 */
public class ShowTailorReportController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(classesList.size()==0 || sexList.size() ==0 || schoolList.size()==0){
			...
			ArrayList<ArrayList<String>> tailorReport= new TailorDAO().generateTailorReport(uniformId, classesList, sexList, schoolList, startDate, endDate);
			
			System.out.println("---- > tailor report --->>>>  " + tailorReport);
			...
			request.setAttribute("NOOFUNIFORM", new TailorDAO().getNumberOfUniforms(uniformId));
					...
					request.getRequestDispatcher("ShowTailorReport.jsp").forward(request, response);
			}
		}
